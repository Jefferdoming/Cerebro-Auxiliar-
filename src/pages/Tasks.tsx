...
export default function Tasks() {
  ...
  const addTask = async () => {
    ...
    await addDoc(collection(db, 'tasks'), {
      ...
      priorityIndex: taskCategory === 'work' ? 1 : taskCategory === 'finance' ? 2 : 3,
      completed: false,
  ...
  const [taskCategory, setTaskCategory] = useState<'work' | 'finance' | 'personal'>('work');
    ...
    <div className="max-w-3xl mx-auto space-y-12">
      <header className="space-y-4">
        ...
        <div className="flex items-center justify-between border-b border-black/5 pb-6">
          ...
          <div className="text-right">
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-80">Status: {tasks.length}/5 Objetivos</span>
          </div>
        </div>
      </header>
      ...
      <div className="space-y-6">
        <div className="flex gap-2 justify-center sm:justify-start">
          {(['work', 'finance', 'personal'] as const).map((cat) => (
            <button
              key={cat}
              ...
              className={cn(
                ...
                taskCategory === cat 
                  ...
                  : "bg-white text-slate-400 border-slate-100 hover:border-brand-primary/30"
              )}
            >
              {cat === 'work' ? 'Trabalho' : cat === 'finance' ? 'Finanças' : 'Pessoal'}
            </button>
          ))}
        </div>
      ...
      <div className="space-y-4">
        {tasks.map((task) => (
          ...
          >
            ...
            <button onClick={() => removeTask(task.id)} className="text-slate-500 hover:text-brand-accent transition-colors">
              <Trash2 size={20} />
            </button>
          </motion.div>
        ))}
        {Array.from({ length: 5 - tasks.length }).map((_, i) => {
          const suggestions = [
            "Beber água & Levantar",
            "Respirar fundo (3x)",
            "Organizar minha mesa",
            "Revisar o próximo passo",
            "5 min de movimento"
          ];
          const suggestion = suggestions[i] || "Slot Disponível";
          ...
          return (
            <motion.div 
              ...
              className="adhd-card !py-6 border-dashed border-2 bg-transparent opacity-10 text-slate-400 text-center font-bold uppercase tracking-widest text-[10px] cursor-pointer hover:border-brand-primary hover:text-brand-primary transition-all flex items-center justify-center gap-3"
            >
              <Plus size={12} />
              {suggestion}
            </motion.div>
